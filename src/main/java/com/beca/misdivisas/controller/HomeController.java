...
public class HomeController {
	   ...
	   //setear en sesion ese idagencia
		...
		try {
			...
			if(login.getVista().equalsIgnoreCase(Constantes.TIPO_VISTA_GENERAL)) {
			    ...
			    logServ.registrarLog(Constantes.MAIN, Constantes.VISTA_GENERAL, Constantes.LOGIN_BECA, true, Util.getRemoteIp(request), usuario);
			}else {
				...
				logServ.registrarLog(Constantes.MAIN, Constantes.VISTA_EMPRESA, Constantes.LOGIN_BECA, true, Util.getRemoteIp(request), usuario);
			}
			...
			if(usuario.getIdUsuario() == null) {
				logs = logRepository.findLogByNombreUsuarioAndAccionAndResultado(usuario.getNombreUsuario().toUpperCase(), Constantes.OPCION_LOGIN, true);
			}else {
				logs = logRepository.findLogByIdUsuarioAndAccionAndResultado(usuario.getIdUsuario(), Constantes.OPCION_LOGIN, true);
			}
			if (logs.size() > 1) {
				Date date = new Date();
			...
			logServ.registrarLog(Constantes.MAIN, e.getLocalizedMessage(), Constantes.LOGIN_BECA, false, Util.getRemoteIp(request), usuario);
	...
	public String main(Login login, Model model) {
		...
		try {
			...
			if (usuario == null) {
				...
				if(tieneAccesoVista(usuario.getPerfilUsuarios(), Constantes.TIPO_VISTA_TODOS)||(tieneAccesoVista(usuario.getPerfilUsuarios(), Constantes.TIPO_VISTA_GENERAL) &&tieneAccesoVista(usuario.getPerfilUsuarios(), Constantes.TIPO_VISTA_EMPRESA))) {
					menues.addAll(menuService.getMenuBy(usuario, Constantes.TIPO_MENU_S, Constantes.TIPO_VISTA_GENERAL));
					menues.addAll(menuService.getMenuBy(usuario, Constantes.TIPO_MENU_U, Constantes.TIPO_VISTA_GENERAL));
				...
				if(usuario.getIdUsuario() == null) {
					logs = logRepository.findLogByNombreUsuarioAndAccionAndResultado(usuario.getNombreUsuario(), Constantes.OPCION_LOGIN, true);
				}else {
					logs = logRepository.findLogByIdUsuarioAndAccionAndResultado(usuario.getIdUsuario(), Constantes.OPCION_LOGIN, true);
				}
				if (logs.size() > 1) {
			...
			logServ.registrarLog(Constantes.MAIN, Constantes.MAIN, Constantes.OPCION_INICIO, true, Util.getRemoteIp(request), usuario);
			if(menues != null)
				model.addAttribute(Constantes.MENUES, menues);
			else
			...
			logServ.registrarLog(retorno, e.getLocalizedMessage(), Constantes.OPCION_INICIO, false, Util.getRemoteIp(request), usuario);
			return Constantes.ERROR;
		}
		return retorno;
	}
	...
	public String cambioDeEmpresa(Login login, Model model) {
		...
		try {
			...
			logServ.registrarLog(Constantes.CAMBIO_EMPRESA, detalle, Constantes.CAMBIO_EMPRESA, true, Util.getRemoteIp(request), usuario);
			
		} catch (Exception e) {
			logServ.registrarLog(Constantes.CAMBIO_EMPRESA, e.getLocalizedMessage(), Constantes.CAMBIO_EMPRESA, false, Util.getRemoteIp(request), usuario);
		}
	...
	/*private List<UsuarioRol> getRoles(LdapUserDetailsImpl principal) {
		...
		for (GrantedAuthority grantedAuthority : authorities) {
			String nombreRol = grantedAuthority.getAuthority().replaceAll(Constantes.ROL_PRE,"");
		 	rol = new Rol();
	...
	private boolean tieneAccesoVista(List<PerfilUsuario> perfilUsuarios, String tipoVista) {
		...
		for (PerfilUsuario perfilUsuario : perfilUsuarios) {
			if(perfilUsuario!=null && perfilUsuario.getPerfil() != null && !perfilUsuario.getPerfil().getTipoVista().isEmpty() && perfilUsuario.getPerfil().getTipoVista().equals(tipoVista))
				resp=true;
		}
