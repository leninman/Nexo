...
public interface IRemesaRepo extends JpaRepository<Remesa, Integer> {
	...
	@Query("SELECT r FROM Remesa r, Sucursal s,  RemesaDetalle rd"
			+ " WHERE r.idSucursal = s.idSucursal and rd.idRemesa = r.idRemesa and rd.idEstatusRemesa=(SELECT MAX(d.idEstatusRemesa) FROM RemesaDetalle d WHERE d.idRemesa=rd.idRemesa) and rd.idEstatusRemesa in (2,4) and r.idOperacion not in (9,12) and s.idEmpresa  = ?1 and rd.idMoneda = ?2  order by rd.fecha desc")
	...
	public List<Remesa> findRemesaByCartaporteAndIdEmpresaByDate(int idEmpresa, String cartaPorte, Date fechaInicio, Date fechaFin);
	...
	public List<ReporteSucursalMapa> getTotalMensualBySucursal(int idSucursal, int moneda, int status, int idOperacion, Date fechaInicio);
